...
export default function ProductList() {
  ...
  const handleFilter = (e, section, option) => {
    ...
    alert.info("Filter Applied!");
  };

  const handleSort = (e, option) => {
    const sort = { _sort: option.sort, _order: option.order };
    setSort(sort);
    alert.info("Sort Applied!");
    // dispatch(getProductsByfilterAsync(newFilter));
  };

  const handlePage = (page) => {
    alert.info("Page Changed");
    setPage(page);
  };
    ...
    <div className="bg-white text-black">
      <div className="mx-auto max-w-2xl px-4 py-0 sm:px-6 sm:py-0 lg:max-w-7xl lg:px-8">
        <div className="mt-1 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
          ...
          {products
            .filter((product) => !product.deleted) // Only show non-deleted products
            .map((product) => (
              <Link key={product.id} to={"/products/" + product.id}>
                <div className="group relative border-2 border-gray-200 p-2 rounded-xl">
                  ...
                  <div className="mt-4 flex justify-between">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">
                        ${Math.min(product.discountedPrice, product.price)}
                      </p>
